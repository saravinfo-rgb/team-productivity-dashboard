...
def rebuild_aggregates(master: pd.DataFrame):
    ...
    customer = master.groupby(["shift_date", "Customer"]).agg(
        tasks=("UID", "count"), actual=("actual", "sum"),
        target=("target", "sum"), eff_hours=("efficitiveHoursDigits", "sum"),
        employees=("UID", "nunique"),
    ).reset_index()
    completed_by_cust = (
        master[master["taskstatus"] == "Completed"]
    ...
    employee = master.groupby(["shift_date", "UID", "Name", "Customer"]).agg(
        tasks=("UID", "count"), actual=("actual", "sum"),
        target=("target", "sum"), eff_hours=("efficitiveHoursDigits", "sum"),
        dept=("department", "first"), cluster=("teamgroup", "first"),
        shift_hours=("shift_hours", "first"),
    ).reset_index()

    # Unified breakdown at Cluster (teamgroup) x Department x Stage x Process x
    # Customer grain. This is the richest table and now backs the KPIs, trend
    # chart, customer chart, touchtime panel, and outflow table — all of which
    # need to respond to the Cluster/Department filters, which only exist at
    # this grain.
    ...
    # User productivity: Cluster x Department x User x Process grain.
    # "Count of article" = distinct real article/ISBN IDs touched (excludes
    # the "0" placeholder used for non-article tasks). Touchtime and
    # non-productivity hours are totals (not averages) here, per request.
    ...
    user_productivity = up.groupby(
        ...
        eff_hours=("efficitiveHoursDigits", "sum"),
        non_prod_seconds=("non_prod_seconds", "sum"),
    ).reset_index()
    ...
    log(f"  daily={len(daily)} customer={len(customer)} employee={len(employee)} "
        f"process_breakdown={len(process_breakdown)} user_productivity={len(user_productivity)}")
    return payload
